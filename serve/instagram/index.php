 ...
 * 投稿内容のファイル
 */
  const INSTA_DATA = '/data.json';

}


function token_update($user_token,$curl_options){
...
function update(){
  ...
  if(file_exists(config::APP_FILENAME)){
    ...
    if($diff >= 2592000) {
      token_update($user_token,$curl_options);
    }
  }else{
    token_update($user_token,$curl_options);
  }
  ...
  $res = curl_exec($ch);
  // echo('<div>'.nl2br(str_replace(" ", "&nbsp;", htmlspecialchars('('.__LINE__.')'."\n".'$res => '.print_r($res, true)))).'</div>');
  file_put_contents(dirname(__FILE__).config::INSTA_DATA, $res);
}
...
//ローカルjson更新時間チェック
if(file_exists(config::INSTA_DATA)){
	$updatetime = filemtime(config::INSTA_DATA);
	$diff = time() - $updatetime;
	if($diff >= 3600) {
		update();
	}
